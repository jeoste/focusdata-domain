...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!RESEND_API_KEY) {
      ...
      return NextResponse.json({ error: "Configuration server missing (RESEND_API_KEY)." }, { status: 500 });
    }
    
    if (!OWNER_EMAIL) {
      console.error("OWNER_EMAIL is missing");
      return NextResponse.json({ error: "Configuration server missing (OWNER_EMAIL)." }, { status: 500 });
    }
    ...
    try {
      const data = await resend.emails.send({
        ...
        to: [OWNER_EMAIL],
        subject: "F O C U S - New subscription to the waitlist",
        html: `
          <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(59,8,70,0.08); padding: 32px;">
            ...
            <p style="font-size: 14px; color: #888; text-align: center;">
              Mail code saved : FOCUSDATADEV888.<br>
            </p>
          </div>
        `
      });
      ...
      if (data.error) {
        ...
        return NextResponse.json({ error: "Error sending email: " + data.error.message }, { status: 500 });
      }
